...
#SBATCH --mem=32G
#SBATCH --gres=gpu:1
#SBATCH --job-name=patch_unet
#SBATCH --output=%j_patch_unet.log

echo "Starting Patch-Based U-Net Training..."
hostname
date
nvidia-smi
...
OUTPUT_DIR="results_patch_unet"

mkdir -p "$OUTPUT_DIR"

echo "Running: run_patch_training.py"
echo "Data: $DATA_DIR"

python "$CODE_DIR/run_patch_training.py" \
    --data_dir "$DATA_DIR" \
