"""
Full Supervision Baseline: ResNet50-UNet (ImageNet Weights)
Data: V3 (Strict HU) - 100% Labeled (221 Training Patients)
...
def get_data_paths(data_dir):
    ...
    # 100% Labeled (All training folders)
    labeled_folders = train_folders
    
    print(f"Full Supervision Split: {len(labeled_folders)} Labeled (100%), {len(val_folders)} Validation")
...
class TransferTrainer:
    ...
    def train(self, data_paths, num_epochs):
        print(f"Starting Full Supervision Training for {num_epochs} epochs...")
    ...
    def plot_progress(self):
        ...
        plt.legend()
        plt.savefig(self.output_dir / 'full_supervision_progress.png')
        plt.close()

    def save_history(self):
        pd.DataFrame(self.history).to_csv(self.output_dir / 'full_supervision_history.csv', index=False)
...
def main():
    ...
    exp_dir = Path(f'experiments/full_supervision_{time.strftime("%Y%m%d_%H%M%S")}')
    exp_dir.mkdir(parents=True, exist_ok=True)
    
    # USE V3 DATA
    paths = get_data_paths(Path('preprocessed_v3'))
