"""
V-Net Training Script (Supervised) with GEMINI Loss
Features:
- Validates on Dice metric
- Tracks Validation Loss (Dice+Focal)
- Saves best model
"""
import sys
import os
...
class DiceFocalComboLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        # V-Net output is ALREADY sigmoid (from vnet.py)
        # But let's act consistently. If vnet output is sigmoid, we shouldn't apply it again?
        # Actually standard practice: Models return logits (linear) for numerical stability.
        # Let's check VNet code: "activation='sigmoid'".
        # OK, so y_pred IS probabilities.
        # We need to adapt loss. Or change VNet to linear.
        # Let's trust my Loss function which expects logits typically?
        # My Loss function in previous scripts:
        # y_pred = tf.nn.sigmoid(y_pred) <-- Expects Logits!
        pass 
        # I will handle this inside the loop or change model.
        # Changing VNet output to None (linear) is safer for Focal Loss stability.
        return 0.0

class DiceFocalComboLossProb(tf.keras.losses.Loss):
    """Expects Probabilities (0-1), not Logits"""
    def __init__(self, dice_weight=0.5, focal_weight=0.5, gamma=2.0, alpha=0.25, smooth=1e-6, **kwargs):
        super().__init__(**kwargs)
        self.dice_weight, self.focal_weight = dice_weight, focal_weight
        self.gamma, self.alpha, self.smooth = gamma, alpha, smooth
...
class VNetTrainer:
    def __init__(self, config, output_dir=None):
        ...
        self.model = create_vnet(
            input_shape=(config.img_size_x, config.img_size_y, config.num_channels)
        )
        # Note: Kursat used Nadam 2e-4. I will use Adam 1e-3 decaying to 1e-5 (Baseline config)
        self.lr_schedule = tf.keras.optimizers.schedules.CosineDecay(1e-3, 50000, alpha=0.01)
        self.optimizer = tf.keras.optimizers.Adam(learning_rate=self.lr_schedule) 
        
        # Model outputs Sigmoid -> Use Probability-based Loss
        self.loss_fn = DiceFocalComboLossProb() 
    ...
    def compute_dice(self, y_true, y_pred_prob):
        y_true = tf.cast(tf.squeeze(y_true), tf.float32)
        y_pred = tf.cast(y_pred_prob > 0.5, tf.float32) # Already sigmoid
    ...
    def train(self, data_paths, num_epochs=30):
        print(f"\nStarting V-Net training for {num_epochs} epochs...")
        ...
        for epoch in range(num_epochs):
            ...
            if val_dice > best_dice:
                best_dice = val_dice
                self.model.save_weights(str(self.output_dir / 'best_vnet.weights.h5'))
        ...
        print(f"\n{'='*60}\nV-Net Complete! Best Dice: {best_dice:.4f}\n{'='*60}")
        self.plot_progress()
        self.save_history()
        return best_dice
    ...
    def plot_progress(self):
        ...
        fig.suptitle('V-Net Training Progress', fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'vnet_progress.png', dpi=150)
        plt.close()
    
    def save_history(self):
        pd.DataFrame({'epoch': range(1, len(self.history['train_loss'])+1), **self.history}).to_csv(
            self.output_dir / 'vnet_history.csv', index=False)
...
def main():
    ...
    exp_dir = Path(f'experiments/vnet_{time.strftime("%Y%m%d_%H%M%S")}')
    exp_dir.mkdir(parents=True, exist_ok=True)
    
    trainer = VNetTrainer(config, output_dir=exp_dir)
