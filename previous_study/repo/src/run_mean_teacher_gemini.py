"""
Mean Teacher SSL with GEMINI Loss (Dice + Focal)
...
class DiceFocalComboLoss(tf.keras.losses.Loss):
    """GEMINI: Dice + Focal Loss Combo (winning loss function)"""
...
class MeanTeacherGemini:
    """Mean Teacher with GEMINI (Dice+Focal) Loss"""
    ...
    def __init__(self, config, ema_decay=0.99, consistency_weight_max=1.0, 
        ...
        print(f"\n=== Mean Teacher + GEMINI (Dice+Focal) ===")
        ...
        # GEMINI Loss for supervised component
        ...
        self.history = {
            ...
            'val_dice': [], 'val_fg_pred': [], 'learning_rate': [], 'consistency_weight': []
        }
    ...
    def train(self, data_paths, num_epochs=30):
        print(f"\nStarting Mean Teacher + GEMINI for {num_epochs} epochs...")
        ...
        for epoch in range(num_epochs):
            ...
            print(f"Time: {epoch_time:.1f}s | Loss: {avg_loss:.4f} | Sup: {avg_sup:.4f} | Cons: {avg_cons:.4f} | Val Dice: {val_dice:.4f} | FG: {fg_pred:.0f}")
            
            if val_dice > best_dice:
                best_dice = val_dice
                self.save_checkpoint('best_mean_teacher_gemini')
        ...
        print(f"\n{'='*60}")
        print(f"Mean Teacher + GEMINI Complete! Best Dice: {best_dice:.4f}")
        print(f"{'='*60}")
        self.plot_progress()
        self.save_history_csv()
        return best_dice
    ...
    def save_checkpoint(self, name):
        checkpoint_dir = Path('mean_teacher_gemini/checkpoints') / time.strftime("%Y%m%d_%H%M%S")
    ...
    def plot_progress(self):
        ...
        fig.suptitle('Mean Teacher + GEMINI (Dice+Focal)', fontweight='bold', fontsize=16, y=0.98)
...
║  Mean Teacher GEMINI (50):   {best_dice:.4f}              ║
╠══════════════════════════════════════════════════╣
...
"""
        ...
        plt.savefig(self.output_dir / 'mean_teacher_gemini_progress.png', dpi=150)
        plt.savefig(self.output_dir / 'mean_teacher_gemini_progress.pdf')
        plt.close()
...
def main():
    ...
    exp_dir = Path(f'experiments/mean_teacher_gemini_{time.strftime("%Y%m%d_%H%M%S")}')
    exp_dir.mkdir(parents=True, exist_ok=True)
    
    trainer = MeanTeacherGemini(
        config,
