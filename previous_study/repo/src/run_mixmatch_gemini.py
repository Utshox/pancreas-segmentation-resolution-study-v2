"""
MixMatch SSL with GEMINI Loss (Dice + Focal)
Applying the winning loss function from supervised experiments
"""
import sys
import os
import numpy as np
...
class MixMatchGemini:
    """MixMatch with GEMINI (Dice+Focal) Loss"""
    ...
    def __init__(self, config, temperature=0.5, alpha=0.75, lambda_u=1.0, 
        ...
        print(f"\n=== MixMatch + GEMINI (Dice+Focal) ===")
        print(f"  Temperature: {temperature}")
        print(f"  MixUp Alpha: {alpha}")
        print(f"  Lambda_u max: {lambda_u}")
        ...
        # GEMINI Loss
        self.supervised_loss = DiceFocalComboLoss(dice_weight=0.5, focal_weight=0.5, gamma=2.0, alpha=0.25)
        ...
        self.history = {
            ...
            'val_dice': [], 'val_fg_pred': [], 'learning_rate': [], 'lambda_u': []
        }
    ...
    @tf.function
    def _sharpen(self, probs, temperature):
        probs_sharp = tf.pow(probs, 1.0/temperature)
        return probs_sharp / (probs_sharp + tf.pow(1-probs, 1.0/temperature) + 1e-7)
    ...
    def _mixup(self, x1, x2, y1, y2, alpha):
        lam = tf.maximum(
            tf.random.uniform([], 0, alpha),
            1 - tf.random.uniform([], 0, alpha)
        )
    ...
    def train(self, data_paths, num_epochs=30):
        print(f"\nStarting MixMatch + GEMINI for {num_epochs} epochs...")
        ...
        for epoch in range(num_epochs):
            ...
            epoch_losses, epoch_sup_losses, epoch_unsup_losses = [], [], []
            ...
            print(f"Time: {epoch_time:.1f}s | Loss: {avg_loss:.4f} | Sup: {avg_sup:.4f} | Unsup: {avg_unsup:.4f} | Val Dice: {val_dice:.4f} | FG: {fg_pred:.0f}")
            
            if val_dice > best_dice:
                best_dice = val_dice
                self.save_checkpoint('best_mixmatch_gemini')
        ...
        print(f"\n{'='*60}")
        print(f"MixMatch + GEMINI Complete! Best Dice: {best_dice:.4f}")
        print(f"{'='*60}")
        self.plot_progress()
        self.save_history_csv()
        return best_dice
    ...
    def save_checkpoint(self, name):
        checkpoint_dir = Path('mixmatch_gemini/checkpoints') / time.strftime("%Y%m%d_%H%M%S")
    ...
    def plot_progress(self):
        ...
        fig.suptitle('MixMatch + GEMINI (Dice+Focal)', fontweight='bold', fontsize=16, y=0.98)
        ...
        ax2.axhline(y=0.7349, color='orange', linestyle=':', label='Supervised (GEMINI): 0.7349')
        ax2.axhline(y=0.7028, color='gray', linestyle=':', alpha=0.5, label='BCE Baseline: 0.7028')
        ...
        ax3.axhline(y=534, color='orange', linestyle=':', label='GT FG pixels (~534)')
        ...
        ax4.axis('off')
        improvement_vs_bce = ((best_dice - 0.7028) / 0.7028) * 100
        improvement_vs_gemini = ((best_dice - 0.7349) / 0.7349) * 100
        summary = f"""
╔══════════════════════════════════════════════════╗
║     MIXMATCH + GEMINI (Dice+Focal)               ║
╠══════════════════════════════════════════════════╣
║  Labels: 50 | Unlabeled: 161 | Val: 70           ║
║  Temperature: {self.temperature}                              ║
║  MixUp Alpha: {self.alpha}                             ║
╠══════════════════════════════════════════════════╣
║  BCE Baseline (221 labels):  0.7028              ║
║  GEMINI Supervised (221):    0.7349              ║
║  MixMatch GEMINI (50):       {best_dice:.4f}              ║
╠══════════════════════════════════════════════════╣
║  vs BCE:    {improvement_vs_bce:+.2f}%                           ║
║  vs GEMINI: {improvement_vs_gemini:+.2f}%                           ║
...
"""
        ...
        plt.savefig(self.output_dir / 'mixmatch_gemini_progress.png', dpi=150)
        plt.savefig(self.output_dir / 'mixmatch_gemini_progress.pdf')
        plt.close()
...
def main():
    ...
    exp_dir = Path(f'experiments/mixmatch_gemini_{time.strftime("%Y%m%d_%H%M%S")}')
    exp_dir.mkdir(parents=True, exist_ok=True)
    
    trainer = MixMatchGemini(
        config,
