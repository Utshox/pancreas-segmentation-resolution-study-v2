"""
CLAUDE Version: Supervised Training with Focal Tversky Loss
Best for small targets (pancreas = 0.2% of image)
α=0.7 (penalize False Negatives - missing pancreas)
β=0.3 (weight for False Positives)
...
def get_all_data_paths(data_dir):
    ...
    # 221 for training, 60 for validation (matching original baseline)
...
class FocalTverskyLoss(tf.keras.losses.Loss):
    """
    Focal Tversky Loss - Best for small targets
    α > β penalizes False Negatives (missing pancreas) more
    Focal component focuses on hard examples
    """
    def __init__(self, alpha=0.7, beta=0.3, gamma=2.0, smooth=1e-6, **kwargs):
...
class SupervisedTrainerClaude:
    """Supervised training with Focal Tversky Loss (CLAUDE version)"""
    
    def __init__(self, config, output_dir=None):
        self.config = config
        self.output_dir = Path(output_dir) if output_dir else Path('.')
        
        print(f"\n=== CLAUDE: Supervised + Focal Tversky Loss ===")
        print(f"  α (FN weight): 0.7")
        print(f"  β (FP weight): 0.3")
        print(f"  γ (focal): 2.0")
        ...
        # FOCAL TVERSKY LOSS
        self.loss_fn = FocalTverskyLoss(alpha=0.7, beta=0.3, gamma=2.0)
    ...
    def train(self, data_paths, num_epochs=30):
        print(f"\nStarting CLAUDE training for {num_epochs} epochs...")
        ...
        for epoch in range(num_epochs):
            ...
            if val_dice > best_dice:
                best_dice = val_dice
                self.save_checkpoint('best_claude')
        ...
        print(f"\n{'='*60}")
        print(f"CLAUDE Training Complete! Best Dice: {best_dice:.4f}")
        print(f"{'='*60}")
        self.plot_progress()
        self.save_history_csv()
        return best_dice
    ...
    def save_checkpoint(self, name):
        checkpoint_dir = Path('supervised_claude/checkpoints') / time.strftime("%Y%m%d_%H%M%S")
    ...
    def plot_progress(self):
        ...
        fig.suptitle('CLAUDE: Supervised + Focal Tversky Loss', fontweight='bold', fontsize=16, y=0.98)
        ...
        ax1.set_ylabel('Focal Tversky Loss')
...
"""
        ...
        plt.savefig(self.output_dir / 'claude_progress.png', dpi=150)
        plt.savefig(self.output_dir / 'claude_progress.pdf')
        plt.close()
    ...
    def save_history_csv(self):
        ...
        df.to_csv(self.output_dir / 'claude_history.csv', index=False)
...
def main():
    ...
    exp_dir = Path(f'experiments/claude_focal_tversky_{time.strftime("%Y%m%d_%H%M%S")}')
    exp_dir.mkdir(parents=True, exist_ok=True)
    
    trainer = SupervisedTrainerClaude(config, output_dir=exp_dir)
    
    best_dice = trainer.train(data_paths, num_epochs=args.num_epochs)
    print(f"\nCLAUDE Results in: {exp_dir}")
