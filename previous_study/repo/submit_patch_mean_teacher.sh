...
#SBATCH --mem=32G
#SBATCH --gres=gpu:1
#SBATCH --job-name=mt_50
#SBATCH --output=%j_mt_50.log

echo "Starting Patch-Based Mean Teacher (SSL) - 50% Labels..."
hostname
date
nvidia-smi
...
OUTPUT_DIR="results_meanteacher_50"

mkdir -p "$OUTPUT_DIR"

echo "Running: run_patch_meanteacher.py"
echo "Labeled Ratio: 0.5 (50%)"

python "$CODE_DIR/run_patch_meanteacher.py" \
    --data_dir "$DATA_DIR" \
