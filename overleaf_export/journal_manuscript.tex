...
\author[vu]{Istiaque Ahmed\corref{cor1}}
\ead{istiaque.ahmed@mif.stud.vu.lt}
\author[vu]{Kursat Komurcu}

\cortext[cor1]{Corresponding author.}
\affiliation[vu]{organization={Institute of Computer Science, Vilnius University},
                  city={Vilnius},
                  country={Lithuania}}

%% Highlights (required by CBM)
\begin{highlights}
\item Preserving native CT resolution (512$\times$512) matters more than architectural complexity for pancreas segmentation
\item A simple 7.8M-parameter patch-based CNN achieves 0.809$\pm$0.015 Dice, competitive with nnU-Net (0.823) and outperforming SAM (0.705)
\item Uncertainty-aware semi-supervised learning recovers 99.3\% of supervised performance with only 50\% of labeled data
\item Fourier analysis proves downsampling destroys 38\% of boundary edge information critical for small-organ delineation
\item Zero-shot transfer to 80 external TCIA volumes confirms SSL provides superior cross-domain generalization
\end{highlights}

\begin{abstract}
\textbf{Background:} The automated segmentation of the pancreas from Computed Tomography (CT) images remains challenging due to the organ's small volume ($<$0.5\% of the abdomen), high anatomical variability, and low contrast with surrounding tissues. Recent trends favor complex architectures (Vision Transformers, foundation models) that often sacrifice spatial resolution to accommodate GPU memory constraints.

\textbf{Methods:} We propose a high-resolution patch-based CNN framework (7.8M parameters) that preserves native CT resolution ($512 \times 512$) through $256 \times 256$ patch extraction with domain-specific HU windowing. We conduct comprehensive ablation studies isolating resolution from architectural complexity, and integrate the framework with three semi-supervised learning (SSL) paradigms. We evaluate against nnU-Net, Vision Transformers, and the Segment Anything Model (SAM/MedSAM), with additional validation on 80 external TCIA volumes.

\textbf{Results:} Our patch-based CNN achieves $0.809 \pm 0.015$ Dice (four independent runs, peak 0.849) on MSD Task07\_Pancreas, competitive with nnU-Net (0.823) and dramatically outperforming SAM (0.705 with GT bounding box prompts) and MedSAM (0.439). Downsampling to $256 \times 256$ or $128 \times 128$ causes IoU collapse from 0.69 to $\sim$0.31. Fourier analysis confirms a 38\% loss in boundary edge strength at $128 \times 128$. Uncertainty-aware Mean Teacher (UA-MT) with 50\% labels recovers 99.3\% of supervised performance, and outperforms the fully supervised model on 80 external TCIA volumes (0.603 vs.\ 0.544 Dice).

\textbf{Conclusions:} For small-organ segmentation, preserving native spatial resolution is more critical than architectural complexity. Data-centric strategies with lightweight CNNs outperform foundation models and are competitive with self-configuring frameworks, while SSL provides superior annotation efficiency and domain generalization.
\end{abstract}

\begin{keyword}
Pancreas segmentation \sep Computed tomography \sep Deep learning \sep Resolution preservation \sep Semi-supervised learning \sep Convolutional neural networks
\end{keyword}
...
The annotation efficiency trend is visualized in Fig.~\ref{fig:efficiency_curve}, which plots approximate Dice performance against label budget for all three SSL methods.
...
\caption{Annotation efficiency curve showing approximate Dice performance as a function of label budget. UA-MT consistently outperforms Mean Teacher and CPS at every data regime. At 50\% labels, UA-MT recovers 99.3\% of fully supervised performance, demonstrating that resolution-preserving SSL is a highly efficient annotation strategy.}
