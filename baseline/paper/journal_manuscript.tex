...
\begin{itemize}
    ...
    \item \textbf{SOTA vs. Complexity:} We demonstrate that a simple patch-based CNN operating at native resolution outperforms both complex Vision Transformers and the self-configuring nnU-Net framework on pancreas segmentation.
    \item \textbf{Annotation Efficiency via SSL:} We integrate our resolution-preserving framework with three Semi-Supervised Learning methods and show that UA-MT recovers 94.5\% of fully supervised performance using only 50\% labeled data.
...
Table~\ref{tab:sota_comparison} summarizes the comparison of our framework against established methods. The self-configuring nnU-Net \cite{nnunet}, trained in its \textit{3d\_fullres} configuration for 1000 epochs, achieved an average Dice of 0.823. While this represents excellent performance, it fell short of our 2D patch-based CNN (0.849 Dice), which benefits from preserving the native in-plane resolution of 0.7~mm. This validates our central hypothesis: maintaining high-resolution 2D spatial features and utilizing an overlapping sliding window during inference is highly competitive with, and in this case superior to, learning native 3D convolutions for small, low-contrast targets.
...
\midrule
\textbf{Ours (Patch CNN)} & \textbf{MSD} & \textbf{0.849} & \textbf{2026} \\
...
To validate clinical utility, we performed full 3D volumetric inference using the sliding-window approach on the champion models. Table~\ref{tab:volumetric_dice} presents the per-case 3D Dice scores. Our fully supervised SOTA achieved a mean 3D Dice of $0.815 \pm 0.087$. The UA-MT model trained on only 50\% of the labels reached $0.803 \pm 0.091$, recovering 94.5\% of the fully supervised performance with half the annotation effort.
...
\midrule
Supervised & 0.902 & 0.845 & 0.696 & 0.816 & 0.815$\pm$0.087 \\
...
Most significantly, on the most challenging case (Pancreas\_005), the UA-MT model trained on 50\% data achieved a Dice of \textbf{0.716}, actually outperforming the fully supervised model (0.696). This finding suggests that uncertainty-aware consistency provides a superior regularizing gradient for ambiguous medical boundaries than voxel-wise labels alone, which can be noisy or inconsistent at low-contrast interfaces. Qualitative multi-slice comparisons across easy, medium, and hard cases are shown in Fig.~\ref{fig:mega_easy}, Fig.~\ref{fig:mega_medium}, and Fig.~\ref{fig:mega_inference_comparison}, respectively.
...
\textbf{Single-Run Results.} Due to computational constraints, the reported results are from single training runs. Standard deviations in Table~\ref{tab:volumetric_dice} are computed across test cases rather than across independent runs. Future work should report mean and standard deviation over multiple runs with different random seeds to assess training stability.
