...
Furthermore, we observed that CPS requires a larger "warm-up" labeled set to stabilize. At 10\% and 25\%, CPS significantly trailed behind UA-MT, as the mutual-supervision between two weakly-initialized networks often leads to representation collapse. Only at 50\% did CPS reach a stable IoU of 0.3997, though it remained inferior to the consistency-based UA-MT framework. Figure~\ref{fig:ssl_curves} visualizes the learning dynamics of these frameworks. The training curves are presented up to Epoch 68, the point at which our highest-performing UA-MT model reached optimal convergence before triggering early-stopping criteria based on allocated computational time limits.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{images/ssl_50_iou_comparison.png}
\caption{Validation IoU curves for the 50\% labeled data regime (shown up to Epoch 68, reflecting the optimal convergence point for the champion UA-MT model). UA-MT demonstrates rapid convergence and superior stability compared to Mean Teacher and Cross-Pseudo Supervision.}
