...
# --- Model Definition (identical to your plot_mega_inference.py) ---
...
def main():
    ...
    # Use a high-performing TCIA case
    tcia_case = "pancreas_ext_052.nii.gz" # This case had Dice 0.8414
    img_path = os.path.join("/scratch/lustre/home/kayi9958/ish/data_external/imagesTs", tcia_case)
    lbl_path = os.path.join("/scratch/lustre/home/kayi9958/ish/data_external/labelsTs", tcia_case)
    
    print(f"Loading TCIA Volume: {tcia_case}...")
    vol_img = nib.load(img_path).get_fdata()
    vol_lbl = nib.load(lbl_path).get_fdata()
    
    # Find slice with largest pancreas
    areas = [np.sum(vol_lbl[:, :, s] > 0) for s in range(vol_lbl.shape[2])]
    best_slice_idx = np.argmax(areas)
    
    # Select 3 slices around the largest pancreas area
    slice_indices = [max(0, best_slice_idx - 5), best_slice_idx, min(vol_lbl.shape[2] - 1, best_slice_idx + 5)]
    
    # Define columns for the plot (GT Green, Models Red)
    columns = [
        "CT Image", 
        "Ground Truth", 
        "Supervised SOTA (MSD)", 
    ...
    fig.suptitle(f"Qualitative Generalization on TCIA Dataset ({tcia_case.replace('.nii.gz', '')})", fontsize=20, fontweight='bold', y=1.02)
    ...
    for row, s_idx in enumerate(slice_indices):
        ...
        preds = [None, slice_lbl, pred_sota_bin, pred_uamt_50_bin]
        
        for col, (ax, pred) in enumerate(zip(axes[row], preds)):
            ax.imshow(slice_img_norm, cmap='gray')
            if pred is not None:
                cmap = 'Greens' if col == 1 else 'Reds' # GT is green, predictions are red
    ...
    out_path = f"baseline/logs/verification/plots/tcia_mega_inference_{tcia_case.replace('.nii.gz', '')}.png"
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
    print(f"Saved TCIA mega plot to {out_path}")
