"""
Multi-Seed UA-MT Semi-Supervised Training
------------------------------------------
Same as run_ssl_uamt.py but with explicit seed control and
standalone student model saving via a custom callback.
"""
...
class SaveStudentCallback(tf.keras.callbacks.Callback):
    """Save standalone student model when val_io_u improves."""
    ...
    def on_epoch_end(self, epoch, logs=None):
        current = logs.get('val_io_u', 0.0)
        if current > self.best_val_iou:
            self.best_val_iou = current
            self.model.student.save(self.save_path)
            print(f"\n  Student saved (val_io_u={current:.4f}) -> {self.save_path}")
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  UA-MT {args.ratio}% TRAINING — SEED {args.seed}")
    print(f"{'='*60}\n")
    ...
    save_student = SaveStudentCallback(os.path.join(out_dir, 'standalone_best.h5'))

    model.fit(train_gen, validation_data=(X_val, Y_val), epochs=EPOCHS,
              callbacks=[save_student,
                         CSVLogger(os.path.join(out_dir, 'log.csv')),
                         EpochCallback()])

    print(f"\nSeed {args.seed} UA-MT training complete. Model saved to {out_dir}")
