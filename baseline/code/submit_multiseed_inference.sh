...
#SBATCH --output=logs/multiseed_inference_%j.log

echo "=== Multi-Seed Inference ==="
...
for SEED in 42 123 456; do
    ...
    # UA-MT 50%
    UAMT_MODEL="baseline/models/multiseed_uamt50/seed_${SEED}/standalone_best.h5"
    if [ -f "$UAMT_MODEL" ]; then
        echo ""
        echo "=== UA-MT 50% Seed $SEED ==="
        date
        python baseline/code/sliding_window_inference.py \
            --image_dir "$IMAGE_DIR" \
            --label_dir "$LABEL_DIR" \
            --model_path "$UAMT_MODEL" \
            --output_dir "$OUTPUT_DIR" \
            --exp_name "uamt50_seed${SEED}"
    else
        echo "SKIP: $UAMT_MODEL not found"
    fi
done
